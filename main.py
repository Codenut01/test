...
from constants import FIND_COINTEGRATED_PAIRS, PLACE_TRADES, RESOLUTION
...
async def calculate_cointegrated_pairs(client):
    ...
    try:
        ...
        store_results = store_cointegration_results(df_market_prices)
        ...
        send_message(f"Error in cointegration recalculation: {e}")
        return False
...
async def main():
    ...
    asyncio.create_task(monitor_and_exit_trades())

    # Run the bot continuously
    while True:
        try:
            # Check if within trading hours
            if not should_trade_based_on_time():
            ...
            send_message(f"Error in main trading loop: {e}")
